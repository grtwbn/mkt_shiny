function(input,output){
    ...
    output$dygraph5 = renderDygraph({
        xts(exposures[, -1], as.Date(exposures[, 1], format = '%m/%d/%Y')) %>%
            dygraph(main = 'Market Returns Relative to Market Exposure') %>%
            dyAxis('x', drawGrid = TRUE) %>%
            dyAxis('y', 
                   label = 'S&P500 10 Year Annualized Total Return  (%)', 
                   valueRange = c('20', '-5'), 
                   independentTicks = TRUE, 
                   #axisLabelColor = '#E0FFFF', # No option to color the axis label TEXT
                   drawGrid = TRUE) %>% 
            dySeries('TR10A', axis = ('y')) %>%
            dyAxis('y2', 
                   label = 'Market Expsoure (%)', 
                   valueRange = c('21.5', '52.5'),
                   independentTicks = TRUE,
                   axisLabelColor = '#D2691E', # No option to color the axis label TEXT
                   drawGrid = FALSE) %>%
            dySeries('EXP', axis = ('y2')) %>%
            dyRangeSelector(height = 20)# %>%
            #dyOptions(colors = brewer.pal(n = 8, name = "Dark2"))
    })
